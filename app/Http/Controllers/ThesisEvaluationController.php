...
class ThesisEvaluationController extends Controller
{
    ...
    if (!in_array($id, $ids)) {
        ...
        $data=Thesisevaluation::where('exam_id',$id)->orderBy('tech_id','asc')->paginate(10);
        $techs=Teacher::orderBy('id','asc')->get();
        $exams=Examcommitteebilling::where('id',$id)->get();
        return view('admin.thesisevaluation.index',['data'=>$data,'techs'=>$techs,'exams'=>$exams,'id' => $id]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate(
            ...
            'tech' => 'required|unique:thesisevaluations,tech_id,NULL,id,exam_id,' . $request->exam,
            'exam' => 'required',

        ]

    );
        $data=new Thesisevaluation();
    ...
    {
        Thesisevaluation::where('id',$request->up_id,)->update([
    ...
    {
        Thesisevaluation::find($request->thesisevaluation_id)->delete();
        return response()->json([
            'statu'=>'success',
        ]);
    }
