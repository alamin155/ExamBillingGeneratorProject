...
use Session;
session_start();
class SupervisionPostGraduateController extends Controller
{
    /**
    ...
    if (!in_array($id, $ids)) {
        ...
        $data=Supervisionpostgraduate::where('exam_id',$id)->orderBy('tech_id','asc')->paginate(10);
        $techs=Teacher::orderBy('id','asc')->get();
        $exams=Examcommitteebilling::where('id',$id)->get();
        return view('admin.supervisionpostgraduate.index',['data'=>$data,'techs'=>$techs,'exams'=>$exams,'id' => $id]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate(
            ...
            'tech' => 'required|unique:supervisionpostgraduates,tech_id,NULL,id,exam_id,' . $request->exam,
            'exam' => 'required',

        ]

    );
        $data=new Supervisionpostgraduate();
    ...
    {
        Supervisionpostgraduate::where('id',$request->up_id,)->update([
    ...
    {
        Supervisionpostgraduate::find($request->supervisionpostgraduate_id)->delete();
        return response()->json([
            'statu'=>'success',
        ]);
    }
