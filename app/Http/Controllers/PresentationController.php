...
class PresentationController extends Controller
{
    ...
    if (!in_array($id, $ids)) {
        ...
        $data=Presentation::where('exam_id',$id)->orderBy('tech_id','asc')->paginate(10);
        $techs=Teacher::orderBy('id','asc')->get();
        $exams=Examcommitteebilling::where('id',$id)->get();
        return view('admin.presentation.index',['data'=>$data,'techs'=>$techs,'exams'=>$exams,'id' => $id]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate(
            ...
            'tech' => 'required|unique:presentations,tech_id,NULL,id,exam_id,' . $request->exam,
            'exam' => 'required',
    ...
    {
        Presentation::where('id',$request->up_id,)->update([
    ...
    {
        Presentation::find($request->presentation_id)->delete();
        return response()->json([
            'statu'=>'success',
        ]);
    }
