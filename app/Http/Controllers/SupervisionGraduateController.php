...
use Session;
session_start();
class SupervisionGraduateController extends Controller
{
    /**
    ...
    if (!in_array($id, $ids)) {
        ...
        $data=Supervisiongraduate::where('exam_id',$id)->orderBy('tech_id','asc')->paginate(10);
        $techs=Teacher::orderBy('id','asc')->get();
        $exams=Examcommitteebilling::where('id',$id)->get();
        return view('admin.supervisiongraduate.index',['data'=>$data,'techs'=>$techs,'exams'=>$exams,'id' => $id]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate(
            ...
            'tech' => 'required|unique:supervisiongraduates,tech_id,NULL,id,exam_id,' . $request->exam,
            'exam' => 'required',

        ]

    );
        $data=new Supervisiongraduate();
    ...
    {
         Supervisiongraduate::where('id',$request->up_id,)->update([
    ...
    {
         Supervisiongraduate::find($request->supervisiongraduate_id)->delete();
        return response()->json([
            'statu'=>'success',
        ]);
    }
