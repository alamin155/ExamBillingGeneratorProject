...
class TabulationController extends Controller
{
    ...
    if (!in_array($id, $ids)) {
        ...
        $data=Tabulation::where('exam_id',$id)->orderBy('tech_id','asc')->paginate(10);
        $techs=Teacher::orderBy('id','asc')->get();
        $exams=Examcommitteebilling::where('id',$id)->get();
        return view('admin.tabulation.index',['data'=>$data,'techs'=>$techs,'exams'=>$exams,'id' => $id]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate(
            ...
            'tech' => 'required|unique:tabulations,tech_id,NULL,id,exam_id,' . $request->exam,
            'exam' => 'required',
    ...
    {
        Tabulation::where('id',$request->up_id,)->update([
    ...
    {
        Tabulation::find($request->tabulation_id)->delete();
        return response()->json([
            'statu'=>'success',
        ]);
    }
