...
<script src="http://cdn.bootcss.com/toastr.js/latest/js/toastr.min.js"></script>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $(document).on('click','.add_moderation',function(e)
      {
        e.preventDefault();
        let tech=$('#tech').val();
        let remk=$('#remk').val();
      ...
      $(document).on('click','.update_moderation_form',function(){
        let id= $(this).data('id');
      ...
      $(document).on('click','.update_moderation',function(e)
      {
        e.preventDefault();
        let up_id = $('#up_id').val();
          ...
          success:function(res){
           // $('select[name="depart"]').val(res.data.up_depart);
            if(res.statu=='success'){
              ...
              $('#updatemoderationForm')[0].reset();
              $('.table').load(location.href+' .table');
              Command: toastr["success"]("Moderation Committee Update Successfully!","success")
      ...
      $(document).on('click','.delete_moderation',function(e)
      {
        e.preventDefault();
        let moderation_id = $(this).data('id');
        
        if(confirm('Are you sure to delete moderation Committee')){
          $.ajax({
          url:"{{route('delete.moderation')}}",
          method:'post',
          ...
          success:function(res){
            if(res.statu=='success'){
              ...
              $('.table').load(location.href+' .table');
              Command: toastr["success"]("Moderation Committee Deleted Successfully!","success")
