...
<script src="http://cdn.bootcss.com/toastr.js/latest/js/toastr.min.js"></script>
  ...
  <script type="text/javascript">
  	$(document).ready(function() {
  		$(document).on('click','.add_scrutinize',function(e)
      {
        e.preventDefault();
        let tech=$('#tech').val();
        ...
        $.ajax({
          url:"{{route('add.scrutinize')}}",
          method:'post',
          data:{tech:tech,exam:exam,numberofstudent:numberofstudent,},
          success:function(res){
            if(res.statu=='success'){
              $('#addModal').modal('hide');
              $('#addscrutinizeForm')[0].reset();
              $('.table').load(location.href+' .table');
              Command: toastr["success"]("Scrutinize Add Successfully!","success")
      ...
      $(document).on('click','.update_scrutinize_form',function(){
        let id= $(this).data('id');
      ...
      $(document).on('click','.update_scrutinize',function(e)
      {
        e.preventDefault();
        let up_id = $('#up_id').val();
          ...
          success:function(res){
           // $('select[name="depart"]').val(res.data.up_depart);
            if(res.statu=='success'){
              ...
              $('#updatescrutinizeForm')[0].reset();
              $('.table').load(location.href+' .table');
              Command: toastr["success"]("Scrutinize Updated Successfully!","success")
      ...
      $(document).on('click','.delete_scrutinize',function(e)
      {
        e.preventDefault();
        let scrutinize_id = $(this).data('id');
        
        if(confirm('Are you sure to delete Scrutinize')){
          $.ajax({
          url:"{{route('delete.scrutinize')}}",
          method:'post',
          data:{scrutinize_id:scrutinize_id},
          success:function(res){
            if(res.statu=='success'){

              $('.table').load(location.href+' .table');
              Command: toastr["success"]("Scrutinize Deleted Successfully!","success")
