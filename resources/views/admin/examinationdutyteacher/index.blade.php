...
<body>
 ...
 <ul class="nav">
     ...
     <div class="container col-6 card-body" style=" margin-left:252px; width:990px; height:650px; background:khaki; border: 2px solid">
      <div class="col-md-14 card-body" style="width:990px">
        <h2 class="my-2 text-center">Examination Duty Teachers</h2>
          <a href="" class="btn btn-primary my-2" data-bs-toggle="modal" data-bs-target="#addModal">Add New Examination Duty Teachers</a>
          @foreach($exams as $d)
              <a href="{{url('examcommitteebilling/'.$d->id.'/show')}} " class="btn btn-success my-2">Goto Back</a>
              @endforeach
      <div class="data-table col-md-10" style="width:985px">
            <table class="table table-bordered" style="border: 2px;">
                        <thead>
                         <tr>
                           <th>SL.</th>
                          <th>Name and Designation</th>
                          <th>No. of Total Exam</th>
                          <th>Exam Duty Carried Out</th>
                          <th>Invigilation</th>
                          <th>Hours</th>
                          <th>Action</th>
                        </tr>
                      ...
                      <tr> 
                       ...
                       <td>{{$d->numberofexam}}</td>
                       <td>{{$d->examdutycarriedout}}</td>
                       <td>
                            @if($d->invigilation==1) Cheif Invigilation @else Invigilation @endif</td>
                       <td>{{$d->hours}}</td>
                       <td>
                      <a href="#" class="btn btn-primary update_examinationdutyteacher_form" 
                      ...
                      data-tech="{{$d->tech_id}}"
                      data-numberofexam="{{$d->numberofexam}}"
                      data-invigilation="{{$d->invigilation}}"
                      data-examdutycarriedout="{{$d->examdutycarriedout}}"
                      data-hours="{{$d->hours}}"
                      >
                        <i class="las la-edit"></i></a>
                      <a href="#" class="btn btn-danger delete_examinationdutyteacher" data-id="{{$d->id}}" >
  ...
  @include('admin.examinationdutyteacher.modal')
  @include('admin.examinationdutyteacher.edit')
  @include('admin.examinationdutyteacher.ajax')
  {!! Toastr::message() !!}
