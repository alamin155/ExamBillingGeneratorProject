...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
  <form method="post" action=""  id="addquestiontypingpublishingForm">
   @csrf
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addModalLabel">Add Question Typing and Publishing</h5>
  ...
  <option  data-item_des="{{$tech->teacher_designation}}" data-item_dep="{{$tech->department->department_name}}" data-item_add="{{$tech->teacher_address}}" value="{{$tech->id}}">{{$tech->teacher_name}} </option>
    @endforeach
      ...
      <div class="form-group">
        <label for="numberofquestion">No. of Questiont</label>
        <input type="number" name="numberofquestion" id="numberofquestion" class="form-control" placeholder="No. of questiont">
      </div>
      <div class="form-group">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary add_questiontypingpublishing">Add Question Typing and Publishing</button>
      </div>
    </div>
  </div>
</form>
